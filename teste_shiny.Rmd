...
output$tsa <- renderTable({
  head(dados_CxLiq %>% filter(data_num == strftime(req(input$dygraph_click$x), "%Y%m%d")))
  })
...
#  %>% filter(data_num = strftime(req(input$dygraph_click$x), "%Y%m%d")
